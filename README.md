...
2. Selectionnez **`Import_comptes_ElomFacture.ema`** (fourni dans ce projet)
3. **Fichier -> Importer -> Format parametrable** -> ce format ->
   selectionnez **`Comptes_SYSCOHADA_ElomFacture.txt`** (les 1091 comptes,
...
3. Selectionnez le fichier **`Import_ecritures_ElomFacture.ema`** fourni dans
...
5. **Fichier -> Importer -> Format parametrable** -> selectionnez ce format
   -> selectionnez votre fichier `.txt` exporte par ElomFacture
...
```
.
├── index.html                          -> l'interface ElomFacture
├── api/extract.js                       -> fonction serverless (garde la cle)
├── Import_ecritures_ElomFacture.ema     -> format Sage pret a charger (ecritures)
├── Import_comptes_ElomFacture.ema       -> format Sage pret a charger (comptes)
├── Comptes_SYSCOHADA_ElomFacture.txt    -> les 1091 comptes SYSCOHADA a importer une fois
├── package.json
├── .gitignore
└── .env.example
```
